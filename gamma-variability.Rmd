...
# To address regional vs local community stability, we probably don't need Wang & Loreau's (2014) complex equations. Instead we can simply compare CVs of total CPUE between local communities and the regional metacommunity:
...
CV <- shallowCPUEArea2 %>% 
  ...
  group_by(area) %>%
  summarise(CV = var(total) / (mean(total)^2)) %>%
  ungroup()


# plot on a log scale
# if anyone wants to make this prettier, please do :)
CV_plot <- ggplot(data=CV, aes(x=area, y = CV)) + 
  geom_point(aes(y = CV), size=2, col=2) +
  
  theme(panel.background = element_blank(),
        axis.line.x = element_line(color="black"),
        axis.line.y = element_line(color="black")) +
  
  scale_y_log10(breaks = c(0.005, 0.01, 0.05)) + # how do I put minor tick marks on the y axis?
...
# Although we can compare local and regional community stability using CVs alone, here we calculate metrics from Wang & Loreau (2014) to help us think about mechanisms underlying our result.
# The following uses formulas from Wang & Loreau 2014 Supp Mat (i.e. for varying biomass within and between local communities).
```{r,  include=FALSE, echo=FALSE, results='hide'}
...
for(i in seq_along(shallowAreas_list)) { # remove list for i = 10
  ...
  #############
  # 1. Calculate averaged species variability (C_list[[i]]$CV_species_i):
  ...
  B_list[[i]] <- shallowAreas_list[[i]] %>%
    ...
    summarise(u_j_i = mean(Mean.totalDensity), # mean biomass (across all years) of species j in patch i
              CV_j_i = var(Mean.totalDensity) / (u_j_i)^2 , # CV of species j in patch i  SHOULD THIS BE SQUARED??? OR NOT??? PROBABLY NOT?
              CV_species_j_i = u_j_i / A_list[[i]]$u_i * CV_j_i) %>% # CV of all species in patch i
    ungroup()
  
  
  # calculate more patch-level stuff
  C_list[[i]] <- B_list[[i]] %>%
  ...
  CV_i_list[[i]] <- (C_list[[i]]$CV_species_i * rho_species_i_list[[i]])^2 # SHOULD THIS BE SQUARED? OR NOT? RETURNS A NEGATIVE VALUE WHEN SPECIES SYNCHRONY WITHIN PATCHES IS NEGATIVE
