...
for(i in seq_along(shallowAreas_list)) { # remove list for i = 10
  ...
  A_list[[i]] <- shallowAreas_list[[i]] %>%
    ...
    ungroup() %>%
    summarise(u_i = mean(annual_total_i)) # mean total community biomass (across all years) for local patch i
...
w_ii <- diag(covMatrix_m) # extract diagonals of the covariance matrix. returns a vector.
  
covMatrix_m[upper.tri(covMatrix_m, diag = TRUE)] <- NA # keep one copy of all the non-diagonals. These are w_ih (temporal covariance of total community biomass between local patches i & h). Note it would be nice to bring in species-level information here, to avoid dominant species from driving the result.
...
rho <- summed_w_ih$sum_w_ih / ((sum(sqrt(w_ii[1:9])))^2)
Beta1 = 1 / rho
...
###Within-patch sums of all between-species covariances:
```{r}
F_list
```
...
###Beta variability (Multiplicative beta variability, i.e. spatial asynchrony):
```{r}
Beta1
```
