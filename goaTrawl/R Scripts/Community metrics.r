...
#######################################################################################
	...
	for(Z in 1:length(BY)){
		out.all	<-	aggregate(dat.fin[,c("Mean","Median","Var")],by=list(MASTER_ID=dat.fin$MASTER_ID,Area=dat.fin$Area,Year=dat.fin$Year,dat.fin[,BY[Z]]),sum)
		colnames(out.all)[4]	<-	c(BY[Z])
