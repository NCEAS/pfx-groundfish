...
setwd(paste(proj.dir,"/_Output plots Pres Abs",sep=""))
...
pdf(paste(SPECIES,"; Pres-Abs Diagnostics; Single Int =",single.intercept,".pdf"),onefile=T)
...
text(paste("N = ",sum(Ntrials),"; Pres = ",sum(Data[,SPECIES],na.rm=T)),x=1,y=3,pos=4)
...
plot(y=DAT.out$y.jit,x=DAT.out$Mean,xlab="Mean Predict",ylab="Observed",main="",axes=F,xlim=x.lim,ylim=y.lim,col=1,cex=0.8)
axis(1)
axis(2,las=2,at=c(0,1))
box(bty="o",lwd=2)
title("Observed vs. Predicted")

# Make a cheesy loess trend line
A			<-	loess(y~Mean,DAT.out,span=0.4)
XXX		<-	seq(0.01, 0.99,by=0.001)
...
if(single.intercept ==FALSE){# Choose single.intercept or multiple.
...
if(single.intercept ==TRUE){# Choose single.intercept or multiple.
	  X.mat	<- cbind(cent.log.depth,cent.log.depth^2)
...
par(mfrow=c(4,3),mar=c(3,3,1,0.5))
	for(Z in 1:length(YEARS)){
	  x.lim = c(3,7)
	  y.lim= c(-0.15,1.15)
	  plot(y=pres.abs[pres.abs$Year == YEARS[Z],SPECIES]+runif(nrow(pres.abs[pres.abs$Year == YEARS[Z],]),-JIT,JIT),
...
} # End if statement for single.intercept
#####
#### PLOT STARTS HERE
if(single.intercept ==TRUE){# Choose single.intercept or multiple.
	par(mfrow=c(1,1),mar=c(3,3,1,0.5))
	  x.lim = c(3,7)
	  y.lim= c(-0.15,1.15)
	  plot(y=pres.abs[pres.abs$Year == YEARS[1],SPECIES]+runif(nrow(pres.abs[pres.abs$Year == YEARS[1],]),-JIT,JIT),
	       x=log(pres.abs$BottomDepth[pres.abs$Year == YEARS[1]]),
	       ylab="Occurrence",axes=F,xlim=x.lim,ylim=y.lim)
	  axis(1)
	  axis(2,las=2,at=seq(0,1,by=0.2))
	  # 	axis(2,las=2,at=c(0,1),labels=c("Abs","Pres"))
...
if(LEAVE.OUT==TRUE){
  ...
  # Make an index to help with the inla objects
  obs.index = inla.stack.index(sdat, "stdata")$data
  pred.index = inla.stack.index(sdat, "stdata.pred")$data
  JIT= 0.15
  ...
  PRED.out	<-	data.frame(cbind(Pred,
                              Mean =  inla.mod$summary.fitted.values$mean[pred.index],
                              Median =  inla.mod$summary.fitted.values$"0.5quant"[pred.index],
                              SD =  inla.mod$summary.fitted.values$sd[pred.index],
                              q.025 = inla.mod$summary.fitted.values$"0.025quant"[pred.index],
                              q.975 = inla.mod$summary.fitted.values$"0.975quant"[pred.index],
                              p.Mean= ANTI.LOGIT(inla.mod$summary.fitted.values$mean[pred.index]),
                              p.Median= ANTI.LOGIT(inla.mod$summary.fitted.values$"0.5quant"[pred.index])))
  PRED.out$y.jit <- PRED.out[,SPECIES] +runif(nrow(PRED.out),-JIT,JIT)
  PRED.out$y     <- PRED.out[,SPECIES]
  
  ### CALCULATE PREDICTIVE ACCURACY ON LEFT OUT OBSERVATIONS.
        # assume predictive distribution is approximately normal on the logit scale
  log.score  <-   PRED.out$y * log(PRED.out$p.Mean) +  (1 - PRED.out$y) * log(1 - PRED.out$p.Mean)
  mean.exp.score <- exp(mean(log.score))
  exp.mean.score <- mean(exp(log.score))
  
  pred.mean   <- prediction(PRED.out$p.Mean,PRED.out$y)
  pred.median <- prediction(PRED.out$p.Median,PRED.out$y)
  
  perf         <- performance(pred.mean, measure = "tpr", x.measure = "fpr")
  acc.perf     <- performance(pred.mean, measure = "acc")
  
  AUC.mean     <- performance(pred.mean, measure = "auc")
  AUC.median   <- performance(pred.median, measure = "auc")
  
  ##### MAKE DIAGNOSTIC PLOTS 
  pdf(paste(SPECIES,"; Pres-Abs Diagnostics;Leave Out=TRUE, Single Int =",single.intercept,".pdf"),onefile=T)
  ...
  text(paste("N.train = ",sum(Ntrials),"; Pres = ",sum(Data[,SPECIES],na.rm=T)),x=1,y=3,pos=4)
  text(paste("N.pred = ",sum(Ntrials.pred),"; Pres = ",sum(Pred[,SPECIES],na.rm=T)),x=1,y=2.8,pos=4)
  
  text(paste("DIC:",round(inla.mod$dic$dic,2),", p.eff = ",round(inla.mod$dic$p.eff,2)),x=1,y=2.6,pos=4)
  text(paste("Mean exp(score):",round(mean.exp.score,3)),,x=1,y=2.4,pos=4)
  text(paste("Exp(mean(score)):",round(exp.mean.score,3)),,x=1,y=2.2,pos=4)
  text(paste("AUC(mean):",round(c(unlist(slot(AUC.mean,"y.values"))),3),"; AUC(median):",round(c(unlist(slot(AUC.median,"y.values"))),3)),x=1,y=2.0,pos=4)
  
  
  text(paste("Run Time =",round(inla.mod$cpu.used[4]/ (60*60),1.8),"hours"),x=1,y=1.4,pos=4)
  text(paste("N.2D knots = ",spde$n.spde),x=1,y=1.6,pos=4)
  
  ##########################################################################################
  ### OBSERVED VERSUS PREDICTED - in sample
  ##########################################################################################
  x.lim=c(0,1)
  y.lim=c(-JIT,1+JIT)
  ...
  axis(2,las=2,at=c(0,1))
  box(bty="o",lwd=2)
  title("Observed vs. Predicted (IN SAMPLE)")
  ...
  ##########################################################################################
  ### OBSERVED VERSUS PREDICTED - Out of sample
  ##########################################################################################
  x.lim=c(0,1)
  y.lim=c(-JIT,1+JIT)
  ...
  par(mfrow=c(1,1),mar=c(4,4,1,0.5))
  plot(y=PRED.out$y.jit,x=PRED.out$p.Mean,xlab="Mean Predict",ylab="Observed",main="",axes=F,xlim=x.lim,ylim=y.lim,col=1,cex=0.8)
  axis(1)
  axis(2,las=2,at=c(0,1))
  box(bty="o",lwd=2)
  title("Observed vs. Predicted (OUT OF SAMPLE)")
  
  # Make a cheesy loess trend line
  A			<-	loess(y~p.Mean,PRED.out,span=0.4)
  XXX		<-	seq(0.01, 0.99,by=0.001)
  B			<-	predict(A, data.frame(p.Mean = XXX), se = TRUE)
  ...
  ##########################################################################################
  ### AUC curves and predictive accuracy plots
  ##########################################################################################
  pred.mean   <- prediction(PRED.out$p.Mean,PRED.out$y)
  pred.median <- prediction(PRED.out$p.Median,PRED.out$y)
  ...
  plot(acc.perf,y.lim=c(0,1),lwd=2)
