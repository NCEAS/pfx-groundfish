...
#	  dat.proj[YEARS.lab] = 0
  ...
  formula = as.formula(paste0("y ~ -1 +",  paste(Covar.names, collapse="+"), 
    ...
    if(model=="binomial") {
       ...
       # lincomb.derived.correlation.matrix included to give Var-Cov matrix of random effects
	     ...
	     Output	<-	 list( Data=subdat,INLA.mod=inlaModel,Covar=Covar,Mesh=mesh1,iset=iset,single.intercept=single.intercept)
	     save(Output,file=paste(species[i],"_binomial_;sing_int=",single.intercept,".Rdata",sep=""))# Save this fitted thing to a workspace	     
  ...
  if(model!="binomial") {
       ...
       inlaModel <- inla(formula, family = "gamma", data=inla.stack.data(sdat),
            ...
            control.inla = list(lincomb.derived.correlation.matrix=TRUE))
       Output	<-	 list( Data=subdat,INLA.mod=inlaModel,Covar=Covar,Mesh=mesh1,iset=iset,single.intercept=single.intercept)
       save(Output,file=paste(species[i],"_pos_;sing_int=",single.intercept,".Rdata",sep=""))# Save this fitted thing to a workspace
