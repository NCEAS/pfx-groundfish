...
```{r, include=FALSE, echo=FALSE}
SSOzipd <- tempfile()
download.file("http://pdx.axiomalaska.com/geoserver/wfs?service=WFS&version=1.0.0&request=GetFeature&outputFormat=SHAPE-ZIP&typeName=axiom:EVOSShorelineSurfaceOiling", SSOzipd, mode="wb")
SSOzip_Unz <- unzip(SSOzipd)

#SSOzip_DB <- SSOzip_L[grep(".mdb", SSOzip_L$Name),]$Name   # creates list of files I want
#UNz <- unzip(SSOzipd, SSOzip_DB) # unzip the two mdb files (makes list I think)
...
#conn <- odbcConnectAccess2007(path.expand("./EVOSShorelineSurfaceOilingLine")) # establish a connection
#SubSurfOil <- sqlFetch(conn,"tbl_DATA_OBS")  # read in a table

#close(conn) 
#unlink(SSOzipd)


getShape <- function(temp_file, URL, shapefile) {
            temp_file <- tempfile()
            download.file(URL, temp_file, mode="wb")
            unzip(temp_file)
            
            shpfledat <- sapply(".", function(f) {fp <- file.path(temp_file, f) 
                                                  return(readOGR(".", shapefile, stringsAsFactors=FALSE))})
            
            }

test_get <- getShape(SSOzipdtest, "http://pdx.axiomalaska.com/geoserver/wfs?service=WFS&version=1.0.0&request=GetFeature&outputFormat=SHAPE-ZIP&typeName=axiom:EVOSShorelineSurfaceOiling", "EVOSShorelineSubSurfaceOilingLine")



slk_sss_test <- readOGR(dsn="SSOzip_Unz",
...
```{r, include=FALSE, echo=FALSE}
AEA_proj <-  "+proj=aea +lat_1=55 +lat_2=65 +lat_0=50 +lon_0=-154 +x_0=0 +y_0=0 +datum=NAD83 +units=m
+no_defs +ellps=GRS80 +towgs84=0,0,0 "


spill_new <- readOGR(dsn="./Oil_Markdown_folder", 
                     layer="Oil_Spill_Boundary_py", 
                     stringsAsFactors=FALSE, 
                     p4s=AEA_proj)

spill_new@data$id = rownames(spill_new@data)

spill_new@proj4string 

LL_proj <-  "+proj=longlat +datum=NAD83"

spill_new <- spTransform(spill_new, LL_proj)

spill_new_pt <- fortify(spill_new, region="id")

spill_new_df <- join(spill_new_pt, spill_new@data, by="id")

```

```{r}
akslk5 <- akmap2 +
          geom_polygon(data=spill_new_df, aes(x=long, y=lat, group=id),
