...
slk_sss_pt <- do.call(rbind,lapply(slk_sss$SegmentID, function(x)data.frame(SegmentID=x,coordinates(slk_sss[slk_sss$SegmentID==x,]))))
...
```{r, include=FALSE, echo=FALSE}
ak_ssso <- akslk3 + 
           geom_path(data=slk_sss,aes(x=X1,y=X2, color=SegmentID), size=3) 
...
```{r, include=FALSE, echo=FALSE}
ak_so <- ak_ssso + 
         geom_path(data=slk_so_df,aes(x=long,y=lat, group=SegmentID), size=3) 
