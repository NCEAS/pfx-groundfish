...
```{r, echo=FALSE}
ak_areas <- akmap2 + 
            geom_point(data=discrete_areas,aes(LONGITUDE,LATITUDE,
                                               color=Area)) + 
            scale_color_manual() +
            theme(legend.key=element_blank())
ak_areas
# make "Area" numeric then use scale_color_gradient
```

```{r, include=FALSE, echo=FALSE}
# Deeper Study Areas
discrete_areas_deep <- read.csv("../goaTrawl/Output Data/goa_discrete_areas_deep_151_300m.csv")
...
ak_areas_deep <- akmap2 +
                 geom_point(data=discrete_areas_deep,
                            aes(LONGITUDE,LATITUDE,color=as.factor(Area))) +
                 # scale_color_manual() +
                 theme(legend.key=element_blank())
ak_areas_deep
