...
# consider instead a Hellinger transformation:
decostand(shallowCPUE_spread3[,2:54],"hell")

ordShallow <- metaMDS(shallowCPUE_spread3[2:54], distance='bray', trymax=1000)
#ordShallow <- metaMDS(decostand(shallowCPUE_spread3[,2:54], "hell"), distance='bray', trymax=1000) # goodness of fit plots look better on data with Hellinger transformation
...
# Deep areas:
ordDeep <- metaMDS(deepCPUE_spread3[2:54], distance='bray', trymax=1000)
ordDeep <- metaMDS(decostand(deepCPUE_spread3[2:54],"hell"), distance='bray', trymax=1000)
...
# Plot shallow areas:
ordiplot(ordShallow, type="n", choices=c(1,2), main=NULL, xlab="NMDS 1", ylab="NMDS 2")#, ylim = c(-0.3, 0.2))
title(main="Shallow Areas")
#text(ordShallow, display = "sites", cex = 0.5, col="black")
text(ordShallow, display = "spec", cex = 0.5, col="black")
#ordiplot(ordShallow, display = "sites") #, groups=shallowCPUE_spread3$Area)
ordihull(ordShallow, groups=shallowCPUE_spread3$Area, show.groups=1, col="red", label=T, lwd=3, lty=1)
ordihull(ordShallow, groups=shallowCPUE_spread3$Area, show.groups=2, col="orange", label=T, lwd=3, lty=1)
ordihull(ordShallow, groups=shallowCPUE_spread3$Area, show.groups=3, col="brown", label=T, lwd=3, lty=1)
ordihull(ordShallow, groups=shallowCPUE_spread3$Area, show.groups=4, col="green", label=T, lwd=3, lty=1)
ordihull(ordShallow, groups=shallowCPUE_spread3$Area, show.groups=5, col="blue", label=T, lwd=3, lty=1)
ordihull(ordShallow, groups=shallowCPUE_spread3$Area, show.groups=6, col="purple", label=T, lwd=3, lty=1)
ordihull(ordShallow, groups=shallowCPUE_spread3$Area, show.groups=7, col="pink", label=T, lwd=3, lty=1)

ordihull(ordShallow, groups=shallowCPUE_spread3$Area, show.groups=8, col="dark grey", label=T, lwd=3, lty=1)
ordihull(ordShallow, groups=shallowCPUE_spread3$Area, show.groups=9, col="black", label=T, lwd=3, lty=1)
...
# Plot deep areas:
ordiplot(ordDeep, type="n", choices=c(1,2), main=NULL, xlab="NMDS 1", ylab="NMDS 2")#, ylim = c(-0.3, 0.2))
#text(ordShallow, display = "sites", cex = 0.5, col="black")
title(main="Deep Areas")
text(ordDeep, display = "spec", cex = 0.5, col="black")
#ordiplot(ordShallow, display = "sites") #, groups=shallowCPUE_spread3$Area)
ordihull(ordDeep, groups=deepCPUE_spread3$Area, show.groups=1, col="red", label=T, lwd=3, lty=1)
ordihull(ordDeep, groups=deepCPUE_spread3$Area, show.groups=2, col="orange", label=T, lwd=3, lty=1)
ordihull(ordDeep, groups=deepCPUE_spread3$Area, show.groups=3, col="brown", label=T, lwd=3, lty=1)
ordihull(ordDeep, groups=deepCPUE_spread3$Area, show.groups=4, col="green", label=T, lwd=3, lty=1)
ordihull(ordDeep, groups=deepCPUE_spread3$Area, show.groups=5, col="blue", label=T, lwd=3, lty=1)
...
shallowDist = vegdist(wisconsin(sqrt(shallowCPUE_spread3[,2:54])),"bray") # consider Hellinger transformation to deal with superabundant vs superrare species (see Legendre & Legendre)
#shallowDist = vegdist(wisconsin(sqrt(shallowCPUE_spread3[,2:54])),"jaccard") 

shallowTest = betadisper(shallowDist,shallowCPUE_spread3$Area) # this gives within-area variability (ie looks at temporal effects ... how different is each year from the mean across all years)
boxplot(shallowTest) # indicates temporal turnover (beta diversity in time)
anova(shallowTest) # p = 0.94   Shows that area of each hull in nmds is not significantly different ... ie no difference in temporal turnover between areas
TukeyHSD(shallowTest)
