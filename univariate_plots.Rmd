...
ggplot(c, aes(year, Mean.totalDensity, group=Area, color=Area))+geom_line()+
  ...
  ylim(min(c$lci), max(c$uci))+ scale_x_continuous(limits=c(1984, 2012), breaks=seq(1984,2012,2))+ggtitle(title)+xlab("Year")+ylab("Mean total density")
}
```
...
sp <- droplevels(sp)
sp.names <- as.list(levels(sp$Species))
```
