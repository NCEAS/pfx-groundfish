...
proj.dir <-"/Users/flipper/Documents/git_repos/pfx-groundfish/goaTrawl/" #getwd()
setwd(proj.dir)
...
if(spp != "Merluccius.productus" & spp!="Hydrolagus.colliei"){# Exclude a couple of species that have missing model components.
  for(mod in c("pos","binomial")) {
    ...
    if(size.data ==FALSE){
          ...
          setwd("/Users/flipper/Dropbox/INLA output/pres/Best")
          DIR <- dir()
        ...
        if(mod!="binomial"){        
          ...
          setwd("/Users/flipper/Dropbox/INLA output/pos/Best")
          DIR <- dir()
          load(DIR[which(substr(DIR,1,this.long)==spp)])
        } 
    }
    ...
    #if(projection=="discrete_areas") myAreas = read.csv("Output Data/Shallow_goa_discrete_areas_for_comparison_Combined(50_to_150m).MH.csv")
    if(projection=="discrete_areas") myAreas = read.csv("Output Data/Deep_goa_discrete_areas_for_comparison_151_to_300m.final.csv")
  ...
  # Write outputDF to file
	setwd(proj.dir)
if(size.data==FALSE){
  	 write.csv(all.sp.pos, file = paste("All_sp_index_meanCPUEByArea.Deep.10.23.csv"), row.names = F)
	   write.csv(all.sp.pres, file = paste("All_sp_index_occurrenceByArea.Deep.10.23.csv"), row.names = F)  
}
if(size.data==TRUE){
	   write.csv(all.sp.pos, file = paste("All_size_sp_index_meanCPUEByArea.Deep.10.23.csv"), row.names = F)
	   write.csv(all.sp.pres, file = paste("All_size_sp_index_occurrenceByArea.Deep.10.23.csv"), row.names = F)  
}
