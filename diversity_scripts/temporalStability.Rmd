...
CV_region <- shallowCPUEArea2 %>% 
             group_by(year) %>%
             summarise(total = sum(Mean.totalDensity)) %>%
             ungroup() %>%
             
             summarize(CV = sd(total) / mean(total))
             

CV_shallow2 <- CV_shallow %>%
               bind_rows(CV_region) %>% 
               mutate(area = ifelse(is.na(area), "Region", area))
...
# 0.174/0.079 # ratio of mean local / regional = 2.202 for shallow areas  # Rachael's results

# plot on a log scale
CV_shallow_plot <- ggplot(data=CV_shallow2, aes(x=area, y = CV)) + 
