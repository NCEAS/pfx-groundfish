...
div_plot3 <- ggplot(dist_df_cat_ar, aes(x=met_ton_km2_ca, y=Exp_B_Div_ca, color=Catch_area)) + 
             ...
             theme_boxplot() + theme(legend.position = c(1,1.25), legend.justification = c(1,1)) + 
...
div_plot3a <- ggplot(dist_df_cat_ar, aes(x=met_ton_km2_ca, y=RaosQ, color=Catch_area)) + 
              ...
              xlab("Fishing Pressure (metric tonnes km-2)") +  scale_color_manual(values=colors_plot) +
              theme_boxplot() + theme(legend.position = "top") + 
...
grid.arrange(div_plot1, div_plot2, div_plot3, div_plot3a, ncol=2, nrow=2)
...
gam1_plot <- ggplot(data=dist_df_cat_ar, aes(x=met_ton_km2_ca, y=Sp_rich_ca, color=Catch_area)) + 
             geom_point(size=6) + ylab("Species Richness") +
...
gam2_plot <- ggplot(data=dist_df_cat_ar, aes(x=met_ton_km2_ca, y=Eff_Num_Sp_ca, color=Catch_area)) + 
             geom_point(size=6) +  ylab("Alpha Diversity") + xlab("Fishing Pressure (metric tonnes km-2)") + 
...
gam3_plot <- ggplot(data=dist_df_cat_ar, aes(x=met_ton_km2_ca, y=Exp_B_Div_ca, color=Catch_area)) + 
             geom_point(size=6) +  ylab("Beta Diversity") + xlab("Fishing Pressure (metric tonnes km-2)") + 
             stat_smooth(method="gam", formula=y ~ s(x, k=4), col="red", size=2) + 
...
gam3a_plot <- ggplot(data=dist_df_cat_ar, aes(x=met_ton_km2_ca, y=RaosQ, color=Catch_area)) + 
              geom_point(size=6) +  ylab("Functional Diversity") + xlab("Fishing Pressure (metric tonnes km-2)") + 
              stat_smooth(method="gam", formula=y ~ s(x, k=4), col="red", size=2) + theme_boxplot() + 
              scale_color_manual(values=colors_plot) + theme(legend.position = "none") +
...
div_plot4c <- ggplot(prod_chl_yr, aes(x=chl_mg_m3_mn, y=Exp_B_Div_mn, color=as.factor(AREA1))) + 
              ...
              stat_smooth(method="lm", col="blue", size=2) + ylab("Beta Diversity") +
              xlab("Primary Productivity (chla mg m^3)") + 
              theme(legend.position = c(0.95,.65), legend.justification = c(0,0),
...
```{r, echo=FALSE, fig.height=9, fig.width=10}
# arranging above plots
grid.arrange(gam4_plot, gam5_plot, gam6_plot, gam7_plot, ncol=2, nrow=2)

```

```{r, echo=FALSE, fig.height=9, fig.width=10}
# arranging above plots
grid.arrange(gam4_plot, gam5_plot, div_plot4c, gam7_plot, ncol=2, nrow=2)
