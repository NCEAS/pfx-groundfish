...
gam1a <- gam(Sp_rich~s(met_ton_km2, k=3), data=dist_df, family=gaussian) 

gam1b <- gam(Sp_rich~s(met_ton_km2, k=40), data=dist_df, family=gaussian) 

gam1c <- gam(Sp_rich~s(met_ton_km2, k=45), data=dist_df, family=gaussian) 
...
gam2a <- gam(Eff_Num_Sp~s(met_ton_km2, k=45), data=dist_df, family=gaussian)

plot(gam2a, pages=1, residuals=TRUE, shade=T)
...
gam3a <- gam(Exp_B_Div~s(met_ton_km2, k=45), data=dist_df, family=gaussian)

plot(gam3a, pages=1, residuals=TRUE, shade=T)
