...
#Code: git_repos/pfx-groundfish/goaTrawl/RScripts/Bootstrap.Diversity.Final.R
...
```{r, include=FALSE, echo=FALSE, warning=FALSE, include=FALSE}
...
ak_areas_all <- akmap2 +
                geom_point(data=discrete_areas, 
                 ...
                 annotate("segment", x=-153.35, y=58.45, xend=-153.35, yend=58.75, color="black", 
                          linetype="dashed", size=1) + 
                 geom_label(aes(x=-158, y=54.5, label="AP"), fill="white", fontface="bold") + 
                 geom_label(aes(x=-151, y=56.5, label="K"), fill="white", fontface="bold") + 
                 geom_label(aes(x=-152.5, y=59.5, label="CI"), fill="white", fontface="bold") + 
                 geom_label(aes(x=-144, y=59.5, label="PWS"), fill="white", fontface="bold")
...
```{r, fig.cap="Locations of study areas (50-150 m) in Gulf of Alaska.", fig.pos="placehere", echo=FALSE}
...
```{r, fig.cap="Alpha diversity, beta diversity and species richness of the groundfish assemblage in shallow areas of the Gulf of Alaska. The medians and confidence intervals estimated from bootstrap sampling are shown.", echo=FALSE, fig.height=10, fig.width=12}
...
```{r, include=FALSE, echo=FALSE, results='hide'}
# Figure 4: NMDS Wisconsin plot
source("../diversity_scripts/nmds.r")

```

```{r, echo=FALSE, fig.height=10, fig.width=12}
ordiplot(ordShallowWiscSqrt, type="n", choices=c(1,2), main=NULL, xlab="NMDS 1", ylab="NMDS 2")
...
```{r, echo=FALSE, fig.height=10, fig.width=12}
gam1_plot_final <- gam1_plot + labs(title="a.") + 
                   theme(axis.title.x = element_blank(), axis.text.x = element_blank()) + 
                   annotate("text", x=0.75, y=13.25, size=3.5, 
                            label=paste("Adj. R sq. =", signif(summary(gam1c)$r.sq, 3),
...
gam2_plot_final <- gam2_plot + labs(title="b.") + theme(axis.title.x = element_blank(), 
                                                        ...
                                                        legend.title = element_blank()) + 
                   annotate("text", x=0.2, y=11.5, size=3.5, 
                            label=paste("Adj. R sq. =", signif(summary(gam2c)$r.sq, 3),
                                        "\np =", signif(summary(gam2c)$s.table[1,4], 3)))

  
gam3_plot_final <- gam3_plot + labs(title="c.") + 
                   annotate("text", x=0.75, y=13.25, size=3.5, 
                            label=paste("Adj. R sq. =", signif(summary(gam3c)$r.sq, 3),
                                        "\np =", signif(summary(gam3c)$s.table[1,4], 3)))

gam3a_plot_final <- gam3a_plot + labs(title="d.") + 
                    annotate("text", x=0.75, y=0.075, size=3.5, 
                             label=paste("Adj. R sq. =", signif(summary(gam3cc)$r.sq, 3),
                                         "\np =", signif(summary(gam3cc)$s.table[1,4], 3)))  

div_plot4c_final <- div_plot4c + labs(title="e.") + 
                    annotate("text", x=1.45, y=14, size=3.5, 
                             label=paste("Adj. R sq. =", signif(summary(lmfit6)$adj.r.squared, 3),
...
```{r, echo=FALSE, fig.height=10, fig.width=12}
# Supplementary Appendix A1: Hellinger NMDS plot
ordiplot(ordShallowHellinger, type="n", choices=c(1,2), xlab="NMDS 1", ylab="NMDS 2")
...
```{r, echo=FALSE, fig.height=10, fig.width=12}
# Supplementary Appendix A2: Productiviy - Disturbance Plots
...
# making final figure
 gam4_plot_final <- gam4_plot + labs(title="a.") + theme(axis.title.x = element_blank(), 
                                                           axis.text.x = element_blank()) + 
                     annotate("text", x=2.35, y=13.25, size=3.5, 
                              label=paste("Adj. R sq. =", signif(summary(gam4a)$r.sq, 3),
                                          "\np =", signif(summary(gam4a)$s.table[1,4], 3)))

 gam5_plot_final <- gam5_plot + labs(title="b.") + 
                    annotate("text", x=2.5, y=11.75, size=3.5, 
                             label=paste("Adj. R sq. =", signif(summary(gam5a)$r.sq, 3),
                                         "\np =", signif(summary(gam5a)$s.table[1,4], 3)))
 
 div_plot4d_final <- div_plot4d + labs(title="c.") + 
                     annotate("text", x=2.5, y=0.09, size=3.5, 
                              label=paste("Adj. R sq. =", signif(summary(lmfit6b)$adj.r.squared, 3),
                                          "\np =", signif(summary(lmfit6b)$coef[2,4], 3)))
 
# div_plot3a_final <- div_plot3a + labs(title="d.")     
# div_plot4c_final <- div_plot4c + labs(title="e.")
# 
 ffig2 <- ggarrange(gam4_plot_final, gam5_plot_final, div_plot4d_final, ncol=2, nrow=2)
# 
 ffig2
...
# Supplementary Appendix A3: figure of correlation between fishing pressure and Chla

# source("./diversity_scripts/Disturbance_Productivity_Hyp.R")

ppfp_final <- ppfp +  
              ylab(expression(paste("Primary Productivity (chla mg ", ~~m^3,")")))  + 
