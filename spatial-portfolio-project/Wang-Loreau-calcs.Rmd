...
allCV <- CV_Area_func(shallowCPUEArea2, agg_list)
#View(allCV)
...
richness1 <- Richness_Area_func(occDat1, agg_list)
#View(richness1)
...
WangLoreauResult <- WangLoreau_func(shallowCPUEArea2, agg_list)
WangLoreauResult
...
bray_func <- function(your_df, pairs){
  ...
  for(j in 1:length(pairs)) {
    ...
    for(k in 1:length(pairs[[j]])) {
       ...
       # create aggregated df's for each k sub-pair of local communities  
