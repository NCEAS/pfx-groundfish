...
deepCPUEArea2 <- left_join(deepCPUEArea, common1, by = "Species") %>% # merge in common names
  select(area, Mean.totalDensity, SD.totalDensity, year, Species, common.name) %>%
  mutate(area = revalue(area, c("1"="10", "2"="11", "3"="13", "4"="14", "5"="12", "Total" = "Region")))
...
CV_shallow_plot <- ggplot(data=CV_shallow, aes(x=area, y = CV)) + 
  ...
  scale_y_log10(breaks = c(0.07, 0.08, 0.09, 0.1, 0.15, 0.2)) + # how do I put minor tick marks on the y axis?
...
# CONSIDER WHETHER TO LOG-TRANSFORMED SPECIES FIRST
