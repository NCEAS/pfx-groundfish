...
# Number of distinct combinations possible for each aggregation:
# 1 community: 10
# 2 communities: 45
# 3: 120
# 4: 210
...
# so, taking 100 random draws (as above) oversamples aggregations of 2, 8, & 9 local communities
# 2 options to deal with this:
# 1) Do 45 random draws for aggregations of 2-8 local communities, 10 random draws for 1 & 9 communities, 1 for 10 communities
# 2) Do 100 random draws for aggregations of 3-7 local communities, 45 for 2 & 8 communities, 10 for 1 & 9 communities, 1 for 10 communities
...
for (i in 1:10) {
  random1_list1[[i]] <- sample(seq(1:10), size = 1)
  random9_list1[[i]] <- sample(seq(1:10), size = 9)
}
for (i in 1:45) {
  random2_list1[[i]] <- sample(seq(1:10), size = 2)
  random3_list1[[i]] <- sample(seq(1:10), size = 3)
  random4_list1[[i]] <- sample(seq(1:10), size = 4)
  random5_list1[[i]] <- sample(seq(1:10), size = 5)
  random6_list1[[i]] <- sample(seq(1:10), size = 6)
  random7_list1[[i]] <- sample(seq(1:10), size = 7)
  random8_list1[[i]] <- sample(seq(1:10), size = 8)
}
random10_list1 <- list(1:10)

random_agg_list_noSingles1 <- c(random2_list1, random3_list1, random4_list1, random5_list1, random6_list1, random7_list1, random8_list1, random9_list1, random10_list1)
#save(random_agg_list_noSingles1, file = "random_agg_list_noSingles1.RData")
load("random_agg_list_noSingles1.RData")
...
for (i in 1:10) {
  random1_list2[[i]] <- sample(seq(1:10), size = 1)
  random9_list2[[i]] <- sample(seq(1:10), size = 9)
}
for (i in 1:45) {
  random2_list2[[i]] <- sample(seq(1:10), size = 2)
  random8_list2[[i]] <- sample(seq(1:10), size = 8)
}
for (i in 1:100) {
  random3_list2[[i]] <- sample(seq(1:10), size = 3)
  random4_list2[[i]] <- sample(seq(1:10), size = 4)
  random5_list2[[i]] <- sample(seq(1:10), size = 5)
  random6_list2[[i]] <- sample(seq(1:10), size = 6)
  random7_list2[[i]] <- sample(seq(1:10), size = 7)
}
random10_list2 <- list(1:10)

random_agg_list_noSingles2 <- c(random2_list2, random3_list2, random4_list2, random5_list2, random6_list2, random7_list2, random8_list2, random9_list2, random10_list2)
#save(random_agg_list_noSingles2, file = "random_agg_list_noSingles2.RData")
load("random_agg_list_noSingles2.RData")
...
# for subdominant species:
CV_Area_spatNested_subdom1 <- CV_Area_func(subdom1, spatially_nested_list_noSingles)
...
CV_Area_randomAgg1 <- CV_Area_func(shallowCPUEArea2, random_agg_list_noSingles1)
CV_Area_randomAgg2 <- CV_Area_func(shallowCPUEArea2, random_agg_list_noSingles2)
#save(CV_Area_randomAgg, file = "CV_Area_randomAgg.RData")
# for subdominant species:
CV_Area_randomAgg1_subdom1 <- CV_Area_func(subdom1, random_agg_list_noSingles1)
...
occDat_subdom1 <- occDat %>%
  select(area, year.numb, Species, Mean.avgPresence) %>%
  filter(!Species %in% c("Atheresthes.stomias", "Hippoglossus.stenolepis", "Gadus.chalcogrammus", "Gadus.macrocephalus"))
...
# for subdominant species:
richness_spatNested_subdom1 <- Richness_func(occDat_subdom1, spatially_nested_list_noSingles)

# run function for random aggregations of local communities:
richness_randomAgg <- Richness_func(occDat1, random_agg_list_noSingles)
richness_randomAgg1 <- Richness_func(occDat1, random_agg_list_noSingles1)
richness_randomAgg2 <- Richness_func(occDat1, random_agg_list_noSingles2)
#save(richness_randomAgg, file = "richness_randomAgg.RData")
# for subdominant species:
richness_randomAgg1_subdom1 <- Richness_func(occDat_subdom1, random_agg_list_noSingles1)


#######################################

# Mean local species richness for each aggregation


meanRichness_func <- function(my_df, spatialGrouping){
  
  unlisted_list <- list()
  mean_richness_list <- list()
  
  # create nested lists with 2 levels:
  loc_rich_list <- vector("list", length(spatialGrouping))
  ...
  for(j in 1:length(spatialGrouping)) {
    ...
    for(k in 1:length(spatialGrouping[[j]])) {
      ...
      # calculate sp richness for each local area
      loc_rich_list[[j]][[k]] <- my_df %>%
                                 filter(area %in% spatialGrouping[[j]]) %>%
                                 filter(area == spatialGrouping[[j]][k]) %>%
      
                                 group_by(year.numb) %>%
                                 summarize(annProbOcc = sum(Mean.avgPresence)) %>% # calculate total prob of occurrence for each year
                                 ungroup() %>%
        
                                 summarize(meanAnnProbOcc = mean(annProbOcc)) # calculate mean across all years
      
    } # end of k
    
    unlisted_list[[j]] <- as.matrix(unlist(loc_rich_list[[j]]))
    mean_richness_list[[j]] <- apply(unlisted_list[[j]], 2, mean) # calculate mean sp richness across all local patches in aggregation j
    
  } # end of j
  
  df_list <- list(mean_richness_list)
  meanRichness_df <- as.data.frame(matrix(unlist(df_list), nrow=length(unlist(df_list[1]))))
  colnames(meanRichness_df) <- c("meanLocalRichness")
  meanRichness_df <- mutate(meanRichness_df, id = rownames(meanRichness_df)) # add column with unique number
  
  return(meanRichness_df)
   
}


# run function for spatially nested aggregations of local communities:
meanRichness_spatNested <- meanRichness_func(occDat1, spatially_nested_list_noSingles)
#meanRichness_spatNested_withSingles <- meanRichness_func(occDat1, spatially_nested_list); meanRichness_spatNested_withSingles
# for subdominant species:
meanRichness_spatNested_subdom1 <- meanRichness_func(occDat_subdom1, spatially_nested_list_noSingles)

# run function for random aggregations of local communities:
meanRichness_randomAgg <- meanRichness_func(occDat1, random_agg_list_noSingles)
meanRichness_randomAgg1 <- meanRichness_func(occDat1, random_agg_list_noSingles1)
meanRichness_randomAgg2 <- meanRichness_func(occDat1, random_agg_list_noSingles2)
#save(meanRichness_randomAgg, file = "meanRichness_randomAgg.RData")
#for subdominant species:
meanRichness_randomAgg1_subdom1 <- meanRichness_func(occDat_subdom1, random_agg_list_noSingles1)


#######################################

# Mean between-patch difference in species richness, for each aggregation:

meanRichDiff_func <- function(my_df, spatialGrouping){
  ...
  for(j in 1:length(spatialGrouping)) {
    ...
    for(k in 1:length(spatialGrouping[[j]])) {
      ...
      # calculate sp richness for each local community k in aggregation j:
      loc_rich_list[[j]][[k]] <- my_df %>%
                                 filter(area %in% spatialGrouping[[j]]) %>%
                                 filter(area == spatialGrouping[[j]][k]) %>%
    ...
    unlisted_list[[j]] <- unlist(loc_rich_list[[j]]) # flatten each j list to a vector
    diffs_list[[j]] <- abs(apply(combn(unlisted_list[[j]], 2), 2, diff)) # for each pair of patches within the aggregation (excluding self pairs), calculate the pairwise difference in local richness
    mean_pairwise_diffs_list[[j]] <- mean(diffs_list[[j]]) # for each aggregation, calculate mean of all pairwise differences in local richness
  ...
  rich_diffs_list <- list(mean_pairwise_diffs_list)
  rich_diffs_df <- as.data.frame(matrix(unlist(rich_diffs_list), nrow=length(unlist(rich_diffs_list[1]))))
  colnames(rich_diffs_df) <- c("meanRichDiff")
  rich_diffs_df <- mutate(rich_diffs_df, id = rownames(rich_diffs_df)) # add column with unique number
    
  
}

# run for spatially-nested aggregations of local communities:
meanRichDiff_spatNested <- meanRichDiff_func(occDat1, spatially_nested_list_noSingles)
# for subdominant species:
meanRichDiff_spatNested_subdom1 <- meanRichDiff_func(occDat_subdom1, spatially_nested_list_noSingles)

# run for random aggregations of local communities:
meanRichDiff_randomAgg <- meanRichDiff_func(occDat1, random_agg_list_noSingles)
meanRichDiff_randomAgg1 <- meanRichDiff_func(occDat1, random_agg_list_noSingles1)
meanRichDiff_randomAgg2 <- meanRichDiff_func(occDat1, random_agg_list_noSingles2)
#save(meanRichDiff_randomAgg, file = "meanRichDiff_randomAgg.RData")
# for subdominant species:
meanRichDiff_randomAgg1_subdom1 <- meanRichDiff_func(occDat_subdom1, random_agg_list_noSingles1)
...
# for subdominant species:
alphaDiv_spatNested_subdom1 <- alphaDiv_func(subdom1, spatially_nested_list_noSingles)

# run for random aggregations of local communities:
alphaDiv_randomAgg <- alphaDiv_func(shallowCPUEArea2, random_agg_list_noSingles)
alphaDiv_randomAgg1 <- alphaDiv_func(shallowCPUEArea2, random_agg_list_noSingles1)
alphaDiv_randomAgg2 <- alphaDiv_func(shallowCPUEArea2, random_agg_list_noSingles2)
#save(alphaDiv_randomAgg, file = "alphaDiv_randomAgg.RData")
# for subdominant species:
alphaDiv_randomAgg1_subdom1 <- alphaDiv_func(subdom1, random_agg_list_noSingles1)


##############################

# Calculate mean local Alpha Diversity

meanAlpha_func <- function(my_df, spatialGrouping){
  
  unlisted_list <- list()
  mean_alpha_list <- list()
  
  # create nested lists with 2 levels:
  loc_alpha_list <- vector("list", length(spatialGrouping))
  ...
  for(j in 1:length(spatialGrouping)) {
    ...
    for(k in 1:length(spatialGrouping[[j]])) {
      ...
      # calculate alpha for each local area (mean across all years)
      loc_alpha_list[[j]][[k]] <- my_df %>%
                                  filter(area %in% spatialGrouping[[j]]) %>%
                                  filter(area == spatialGrouping[[j]][k]) %>%
        
                                  group_by(year) %>%
                                  summarize(SW_Div = diversity(Mean.totalDensity, index="shannon"), 
                                            Exp_A_Div = exp(SW_Div)) %>% # calculate alpha for each year ... should this be calculated on raw or log abundances???
                                  ungroup() %>% # note between-year variation in Exp_A_Div
    ...
    unlisted_list[[j]] <- as.matrix(unlist(loc_alpha_list[[j]]))
    mean_alpha_list[[j]] <- apply(unlisted_list[[j]], 2, mean) # calculate mean alpha across all local patches in aggregation j
    
  } # end of j
  
  
  df_list <- list(mean_alpha_list)
  meanAlpha_df <- as.data.frame(matrix(unlist(df_list), nrow=length(unlist(df_list[1]))))
  colnames(meanAlpha_df) <- c("meanLocalExpAlpha")
  meanAlpha_df <- mutate(meanAlpha_df, id = rownames(meanAlpha_df)) # add column with unique number
...
# run for spatially-nested aggregations of local communities:
meanAlpha_spatNested <- meanAlpha_func(shallowCPUEArea2, spatially_nested_list_noSingles)
# for subdominant species:
meanAlpha_spatNested_subdom1 <- meanAlpha_func(subdom1, spatially_nested_list_noSingles)


# run for random aggregations of local communities:
meanAlpha_randomAgg <- meanAlpha_func(shallowCPUEArea2, random_agg_list_noSingles)
meanAlpha_randomAgg1 <- meanAlpha_func(shallowCPUEArea2, random_agg_list_noSingles1)
meanAlpha_randomAgg2 <- meanAlpha_func(shallowCPUEArea2, random_agg_list_noSingles2)
#save(meanAlpha_randomAgg, file = "meanAlpha_randomAgg.RData")
# for subdominant species:
meanAlpha_randomAgg1_subdom1 <- meanAlpha_func(subdom1, random_agg_list_noSingles1)
...
sync_within_func <- function(my_df, spatialGrouping){
  ...
  unlisted_loreau_list <- list()
  unlisted_gross_list <- list()
  unlisted_thibaut_list <- list()
  ...
  for(j in 1:length(spatialGrouping)) {
   ...
   ###################### 
      ...
      AAAA_list[[j]][[k]] <- AAA_list[[j]][[k]] %>%
                             select(-area, -SD.totalDensity, -km2, -Species) %>%
      ...
      unlisted_loreau_list[[j]] <- as.matrix(unlist(loreau_list[[j]]))
      loreau_means_list[[j]] <- apply(unlisted_loreau_list[[j]], 2, mean) # calculate mean across all local patches in aggregation j
      
      unlisted_gross_list[[j]] <- as.matrix(unlist(gross_list[[j]]))
      gross_means_list[[j]] <- apply(unlisted_gross_list[[j]], 2, mean) # calculate mean across all local patches in aggregation j
      
      unlisted_thibaut_list[[j]] <- as.matrix(unlist(thibaut_list[[j]]))
      thibaut_means_list[[j]] <- apply(unlisted_thibaut_list[[j]], 2, mean) # calculate mean across all local patches in aggregation j
...
#save(sync_within_spatNested, file = "sync_within_spatNested.RData")
#load(sync_within_spatNested.RData)
# for subdominant species:
sync_within_spatNested_subdom1 <- sync_within_func(subdom1, spatially_nested_list_noSingles)
#save(sync_within_spatNested_subdom1, file = "sync_within_spatNested_subdom1.RData")


# run for random aggregations of local communities:
sync_within_randomAgg <- sync_within_func(shallowCPUEArea2, random_agg_list_noSingles)#; save(sync_within_randomAgg, file = "sync_within_randomAgg.RData")
sync_within_randomAgg1 <- sync_within_func(shallowCPUEArea2, random_agg_list_noSingles1)#; save(sync_within_randomAgg1, file = "sync_within_randomAgg1.RData")
sync_within_randomAgg2 <- sync_within_func(shallowCPUEArea2, random_agg_list_noSingles2)#; save(sync_within_randomAgg2, file = "sync_within_randomAgg2.RData")
#load("sync_within_randomAgg.RData")

# for subdominant species:
sync_within_randomAgg1_subdom1 <- sync_within_func(subdom1, random_agg_list_noSingles1)
#save(sync_within_randomAgg1_subdom1, file = "sync_within_randomAgg1_subdom1.RData")
...
spat_uneveness_func <- function(my_df, spatialGrouping){
  ...
  colnames(unev_df) <- c("meanUneveness")
...
# for subdominant species:
uneveness_spatNested_subdom1 <- spat_uneveness_func(subdom1, spatially_nested_list_noSingles)


# run for random aggregations of local communities:
uneveness_randomAgg <- spat_uneveness_func(shallowCPUEArea2, random_agg_list_noSingles)
uneveness_randomAgg1 <- spat_uneveness_func(shallowCPUEArea2, random_agg_list_noSingles1)
uneveness_randomAgg2 <- spat_uneveness_func(shallowCPUEArea2, random_agg_list_noSingles2)
#save(uneveness_randomAgg, file = "uneveness_randomAgg.RData")
# for subdominant species:
uneveness_randomAgg1_subdom1 <- spat_uneveness_func(subdom1, random_agg_list_noSingles1)
...
# for subdominant species:
sync_between_spatNested_subdom1 <- sync_between_func(subdom1, spatially_nested_list_noSingles)


# run function for random aggregations of local communities:
# note I must remove single local communities to look at synchrony between communities
sync_between_randomAgg <- sync_between_func(shallowCPUEArea2, random_agg_list_noSingles)
sync_between_randomAgg1 <- sync_between_func(shallowCPUEArea2, random_agg_list_noSingles1)
sync_between_randomAgg2 <- sync_between_func(shallowCPUEArea2, random_agg_list_noSingles2)
#save(sync_between_randomAgg, file = "sync_between_randomAgg.RData")
# for subdominant species:
sync_between_randomAgg1_subdom1 <- sync_between_func(subdom1, random_agg_list_noSingles1)
...
avSpeciesVar_within_func <- function(my_df, spatialGrouping){
  ...
  for(j in 1:length(spatialGrouping)) {
    ...
    A_list[[j]] <- AAA_list[[j]] %>%
      ...
      ungroup() %>%
      summarize(#var_i = var(annual_total_i),  # remnant code; unnecessary now
                #stDev_i = sqrt(var(annual_total_i)),  # remnant code; unnecessary now
  ...
  # calculate averaged species variability
    C_list[[j]] <- B_list[[j]] %>%
      summarise(CV_species_i_squared = sum(CV_species_j_i)) # using formula from Wang & Loreau 2014 ... but this is a sum, not a mean ...?
...
# for subdominant species:
avSpVar_within_spatNested_subdom1 <- avSpeciesVar_within_func(subdom1, spatially_nested_list_noSingles)


# run function for random aggregations of local communities:
avSpVar_within_randomAgg <- avSpeciesVar_within_func(shallowCPUEArea2, random_agg_list_noSingles)
avSpVar_within_randomAgg1 <- avSpeciesVar_within_func(shallowCPUEArea2, random_agg_list_noSingles1)
avSpVar_within_randomAgg2 <- avSpeciesVar_within_func(shallowCPUEArea2, random_agg_list_noSingles2)
#save(avSpVar_within_randomAgg, file = "avSpVar_within_randomAgg.RData")
# for subdominant species:
avSpVar_within_randomAgg1_subdom1 <- avSpeciesVar_within_func(subdom1, random_agg_list_noSingles1)
...
# for subdominant species:
altBrayResult_spatNested_subdom1 <- alt_bray_func(subdom1, spatially_nested_list_noSingles)


# run function for random aggregations of local communities:
altBrayResult_randomAgg <- alt_bray_func(shallowCPUEArea2, random_agg_list_noSingles)
altBrayResult_randomAgg1 <- alt_bray_func(shallowCPUEArea2, random_agg_list_noSingles1)
altBrayResult_randomAgg2 <- alt_bray_func(shallowCPUEArea2, random_agg_list_noSingles2)
#save(altBrayResult_randomAgg, file = "altBrayResult_randomAgg.RData")
# for subdominant species:
altBrayResult_randomAgg1_subdom1 <- alt_bray_func(subdom1, random_agg_list_noSingles1)
...
randomAgg <- CV_Area_randomAgg %>%
  ...
  left_join(meanRichness_randomAgg, by = "id") %>%
  left_join(meanRichDiff_randomAgg, by = "id") %>%
  #left_join(alphaDiv_randomAgg, by = "id") %>%
  left_join(meanAlpha_randomAgg, by = "id") %>%
...
randomAgg1 <- CV_Area_randomAgg1 %>%
  left_join(richness_randomAgg1, by = "id") %>%
  left_join(meanRichness_randomAgg1, by = "id") %>%
  left_join(meanRichDiff_randomAgg1, by = "id") %>%
  #left_join(alphaDiv_randomAgg1, by = "id") %>%
  left_join(meanAlpha_randomAgg1, by = "id") %>%
  left_join(sync_within_randomAgg1, by = "id") %>%
  left_join(sync_between_randomAgg1, by = "id") %>%
  left_join(avSpVar_within_randomAgg1, by = "id") %>%
  left_join(uneveness_randomAgg1, by = "id") %>%
  #left_join(brayResult_randomAgg1, by = "id") %>%
  left_join(altBrayResult_randomAgg1, by = "id") %>%
  mutate(n = as.character(n))
#save(randomAgg1, file = "randomAgg1.RData")

randomAgg2 <- CV_Area_randomAgg2 %>%
  left_join(richness_randomAgg2, by = "id") %>%
  left_join(meanRichness_randomAgg2, by = "id") %>%
  left_join(meanRichDiff_randomAgg2, by = "id") %>%
  #left_join(alphaDiv_randomAgg2, by = "id") %>%
  left_join(meanAlpha_randomAgg2, by = "id") %>%
  left_join(sync_within_randomAgg2, by = "id") %>%
  left_join(sync_between_randomAgg2, by = "id") %>%
  left_join(avSpVar_within_randomAgg2, by = "id") %>%
  left_join(uneveness_randomAgg2, by = "id") %>%
  #left_join(brayResult_randomAgg2, by = "id") %>%
  left_join(altBrayResult_randomAgg2, by = "id") %>%
  mutate(n = as.character(n))
#save(randomAgg2, file = "randomAgg2.RData")


randomAgg1_subdom1 <- CV_Area_randomAgg1_subdom1 %>%
  left_join(richness_randomAgg1_subdom1, by = "id") %>%
  left_join(meanRichness_randomAgg1_subdom1, by = "id") %>%
  left_join(meanRichDiff_randomAgg1_subdom1, by = "id") %>%
  #left_join(alphaDiv_randomAgg1_subdom1, by = "id") %>%
  left_join(meanAlpha_randomAgg1_subdom1, by = "id") %>%
  left_join(sync_within_randomAgg1_subdom1, by = "id") %>%
  left_join(sync_between_randomAgg1_subdom1, by = "id") %>%
  left_join(avSpVar_within_randomAgg1_subdom1, by = "id") %>%
  left_join(uneveness_randomAgg1_subdom1, by = "id") %>%
  #left_join(brayResult_randomAgg1_subdom1, by = "id") %>%
  left_join(altBrayResult_randomAgg1_subdom1, by = "id") %>%
  mutate(n = as.character(n))
#save(randomAgg1_subdom1, file = "randomAgg1_subdom1.RData")
...
spatNested <- CV_Area_spatNested %>%
  ...
  left_join(meanRichness_spatNested, by = "id") %>%
  left_join(meanRichDiff_spatNested, by = "id") %>%
  #left_join(alphaDiv_spatNested, by = "id") %>%
  left_join(meanAlpha_spatNested, by = "id") %>%
...
spatNested_subdom1 <- CV_Area_spatNested_subdom1 %>%
  left_join(richness_spatNested_subdom1, by = "id") %>%
  left_join(meanRichness_spatNested_subdom1, by = "id") %>%
  left_join(meanRichDiff_spatNested_subdom1, by = "id") %>%
  #left_join(alphaDiv_spatNested_subdom1, by = "id") %>%
  left_join(meanAlpha_spatNested_subdom1, by = "id") %>%
  left_join(sync_within_spatNested_subdom1, by = "id") %>%
  left_join(sync_between_spatNested_subdom1, by = "id") %>%
  left_join(avSpVar_within_spatNested_subdom1, by = "id") %>%
  left_join(uneveness_spatNested_subdom1, by = "id") %>%
  #left_join(brayResult_spatNested_subdom1, by = "id") %>%
  left_join(altBrayResult_spatNested_subdom1, by = "id") %>%
  mutate(n = as.character(n))
#save(spatNested_subdom1, file = "spatNested_subdom1.RData")
...
spatNested_indepVar <- spatNested %>%
  select(Area, meanLocalRichness, meanRichDiff, meanLocalExpAlpha, grossSyncWithin, avSpVar, meanUneveness, grossSyncBetween, meanBray)
...
randomAgg1_indepVar <- randomAgg1 %>%
  select(Area, meanLocalRichness, meanRichDiff, meanLocalExpAlpha, grossSyncWithin, avSpVar, meanUneveness, grossSyncBetween, meanBray)
pairs.panels(randomAgg1_indepVar,smooth=F,density=T,ellipses=F,lm=T,digits=3,scale=T, cor = T, rug=T, main = "Random Aggregations 1 of Local Communities")
...
SN_cor_grossSyncWithin_bray <- ggplot(data=spatNested, aes(x=grossSyncWithin, y = meanBray, color = n, shape = n)) + 
  geom_point(size=3, show.legend = FALSE) +
  theme_1_noLegend() +
  geom_smooth(method = lm, se = F, size = 1) +
  scale_color_manual(values = colorsN, limits = catN) +
...
SN_cor_grossSyncBetween_bray <- ggplot(data=spatNested, aes(x=grossSyncBetween, y = meanBray, color = n, shape = n)) + 
  geom_point(size=3, show.legend = FALSE) +
  theme_1_noLegend() +
  geom_smooth(method = lm, se = F, size = 1) +
  scale_color_manual(values = colorsN, limits = catN) +
...
#avSpVar, bray
SN_cor_avSpVar_bray <- ggplot(data=spatNested, aes(x=avSpVar, y = meanBray, color = n, shape = n)) + 
  geom_point(size=3, show.legend = FALSE) +
  theme_1_noLegend()+
  geom_smooth(method = lm, se = F, size = 1) +
  scale_color_manual(values = colorsN, limits = catN) +
...
# grossSyncWithin, bray
RA_cor_grossSyncWithin_bray <- ggplot(data=randomAgg, aes(x=grossSyncWithin, y = meanBray, color = n, shape = n)) + 
  geom_point(size=2, show.legend = FALSE) +
  theme_1_noLegend() +
  geom_smooth(method = lm, se = F, size = 1) +
  scale_color_manual(values = colorsN, limits = catN) +
...
#grossSyncBetween, bray
RA_cor_grossSyncBetween_bray <- ggplot(data=randomAgg, aes(x=grossSyncBetween, y = meanBray, color = n, shape = n)) + 
  geom_point(size=2, show.legend = FALSE) +
  theme_1_noLegend() +
  geom_smooth(method = lm, se = F, size = 1) +
  scale_color_manual(values = colorsN, limits = catN) +
...
#avSpVar, bray
RA_cor_avSpVar_bray <- ggplot(data=randomAgg, aes(x=avSpVar, y = meanBray, color = n, shape = n)) + 
  geom_point(size=2, show.legend = FALSE) +
  theme_1_noLegend() +
  geom_smooth(method = lm, se = F, size = 1) +
  scale_color_manual(values = colorsN, limits = catN) +
...
SN_cvGross_within <- ggplot(data=spatNested, aes(x=grossSyncWithin, y = CV, color = n, shape = n)) +
  #ggplot(data=spatNested, aes(x=grossSyncWithin, y = CV)) + theme_1_noLegend() +
  geom_point(size=3, show.legend = FALSE) +
  theme_1_withLegend() +
...
SN_cvGross_between <- ggplot(data=spatNested, aes(x=grossSyncBetween, y = CV, color = n, shape = n)) +
  #ggplot(data=spatNested, aes(x=grossSyncBetween, y = CV)) +
  geom_point(size=3) +
  theme_1_noLegend() +
  scale_x_reverse() +
  geom_smooth(method = lm, se = F, size = 0.75) +
...
SN_cv_AvSpVar_within <- ggplot(data=spatNested, aes(x=avSpVar, y = CV, color = n, shape = n)) +
  #ggplot(data=spatNested, aes(x=avSpVar, y = CV)) +
  geom_point(size=3) +
  theme_1_noLegend() +
  geom_smooth(method = lm, se = F, size = 0.75) +
  scale_color_manual(values = colorsN, limits = catN) +
...
SN_CV_bray_plot <- ggplot(data=spatNested, aes(x=meanBray, y = CV, color = n, shape = n)) +
  #ggplot(data=spatNested, aes(x=meanBray, y = CV)) + 
  geom_point(size=3) +
  theme_1_noLegend() +
  geom_smooth(method = lm, se = F, size = 0.75) +
  scale_color_manual(values = colorsN, limits = catN) +
...
summary(lm(spatNested$meanBray ~ spatNested$CV))
# Multiple R-squared:  0.3255,	Adjusted R-squared:  0.3098 
# F-statistic: 20.75 on 1 and 43 DF,  p-value: 4.281e-05
...
SN_CV_SpRich <- ggplot(data=spatNested, aes(x=regionalSpRichness, y = CV, color = n, shape = n)) + 
  ...
  scale_color_manual(values = colorsN, limits = catN) +
  scale_shape_manual(values = shapesN, limits = catN) +
  labs(x = "Regional Species Richness
       (Probability of occurrence in at least one patch)",
       y = "CV")
...
SN_Bray_SpRich <- ggplot(data=spatNested, aes(x=regionalSpRichness, y = meanBray, color = n, shape = n)) + 
  geom_point(size=3, show.legend = F) +
  theme_1_noLegend() +
  geom_smooth(method = lm, se = F, size = 0.75) +
  scale_color_manual(values = colorsN, limits = catN) +
...
# plot CV vs synchrony within local communities (between species)
RA_cvGross_within <- ggplot(data=randomAgg, aes(x=grossSyncWithin, y = CV, color = n, shape = n)) + theme_1_withLegend() +
  #ggplot(data=randomAgg, aes(x=grossSyncWithin, y = CV)) + theme_1_noLegend() +
  geom_point(size=2, show.legend = FALSE) +
  scale_x_reverse(breaks = c(0.05, 0.0, -0.05, -0.1, -0.15), labels = c(0.05, 0.0, -0.05, -0.1, -0.15)) +
  geom_smooth(method = lm, se = F, size = 1) +
  scale_color_manual(values = colorsN, limits = catN) +
...
RA_cvGross_between <- ggplot(data=randomAgg, aes(x=grossSyncBetween, y = CV, color = n, shape = n)) + 
  #ggplot(data=randomAgg, aes(x=grossSyncBetween, y = CV)) +
  geom_point(size=2) +
  theme_1_noLegend() +
  scale_x_reverse() +
  geom_smooth(method = lm, se = F, size = 1) +
...
RA_cv_AvSpVar_within <- ggplot(data=randomAgg, aes(x=avSpVar, y = CV, color = n, shape = n)) + 
  #ggplot(data=randomAgg, aes(x=avSpVar, y = CV)) +
  geom_point(size=2) +
  theme_1_noLegend() +
  geom_smooth(method = lm, se = F, size = 1) +
  scale_color_manual(values = colorsN, limits = catN) +
...
# CV vs Spatial Turnover:
RA_CV_bray_plot <- ggplot(data=randomAgg, aes(x=meanBray, y = CV, color = n, shape = n)) + 
  #ggplot(data=randomAgg, aes(x=meanBray, y = CV)) + 
  geom_point(size=2) +
  theme_1_noLegend() +
  geom_smooth(method = lm, se = F, size = 1) +
  scale_color_manual(values = colorsN, limits = catN) +
  scale_shape_manual(values = shapesN, limits = catN) +
  labs(x = "Bray Dissimilarity", y = "CV (Total Community CUE)")
summary(lm(randomAgg$CV ~ randomAgg$meanBray))
# Residual standard error: 0.02198 on 799 degrees of freedom
# Multiple R-squared:  0.1588,	Adjusted R-squared:  0.1577 
# F-statistic: 150.8 on 1 and 799 DF,  p-value: < 2.2e-16
...
RA_CV_SpRich <- ggplot(data=randomAgg, aes(x=regionalSpRichness, y = CV, color = n, shape = n)) +
  #ggplot(data=randomAgg, aes(x=regionalSpRichness, y = CV)) +
  geom_point(size=5, show.legend = FALSE) +
  theme(panel.background = element_blank(),
        axis.line.x = element_line(color="black"),
        axis.line.y = element_line(color="black"),
...
RA_Bray_SpRich <- ggplot(data=randomAgg, aes(x=regionalSpRichness, y = meanBray, color = n, shape = n)) + 
  geom_point(size=2, show.legend = F) +
  theme_1_noLegend() +
  geom_smooth(method = lm, se = F, size = 0.75) +
  scale_color_manual(values = colorsN, limits = catN) +
