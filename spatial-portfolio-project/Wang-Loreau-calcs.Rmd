...
Richness_func <- function(my_df, spatialGrouping){
  ...
  colnames(spRichness_df) <- c("speciesRichness")
...
alphaDiv_func <- function(my_df, spatialGrouping){
  ...
  for(j in 1:length(spatialGrouping)) {
    ...
    group_by(year, Species) %>% # take mean abund across all sites? summed abund?  
    ...
    ungroup() %>% # note Exp_A_Div varies between years
