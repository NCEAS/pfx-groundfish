...
WangLoreau_func <- function(this_df, spatialGrouping){
  ...
  u_m_list <- list(); alpha_CV_list <- list()
  ...
  for(i in seq_along(spatialGrouping)) {
    ...
    for(j in seq_along(unique(sort(AA_list[[i]]$area)))) { # loop over each unique local area j within spatialGrouping in AA_list[[i]]
      
      AAA_list[[j]] <- AA_list[[i]] %>%
         filter(area == j) 
  ...
  # calculate species-level stuff
    B_list[[j]] <- AAA_list[[j]] %>%
      ...
      summarise(u_j_i = mean(Mean.totalDensity), # mean biomass (across all years) of species j in local community i
                ...
                CV_species_j_i = u_j_i / A_list[[j]]$u_i * CV_j_i_squared) %>% # CV of species in local community i
      ungroup()
  ...
  # 2. Calculate species asynchrony WITHIN local areas (rho_species_i_list[[i]]):
    ...
    covMatrix_i_list[[j]] <- cov(D_list[[j]]) # create the covariance matrix
    
    w_kk_i_list[[j]] <- diag(covMatrix_i_list[[j]]) # extract diagonals of the covariance matrix. these are temporal variance of each species k within local community i
    
    covMatrix_i_list[[j]][upper.tri(covMatrix_i_list[[j]], diag = TRUE)] <- NA # for each local community i, keep one copy of all the non-diagonals. these are w_kl_i
    E_list[[j]] <- as.data.frame(covMatrix_i_list[[j]])
    
    F_list[[j]] <- E_list[[j]] %>%
      summarise(sum_w_kl_i = sum(E_list[[j]][,1:53], na.rm = T)) # for each local community i, find the sum of all w_kl_i. calc has been checked
    
    rho_species_i_list[[j]] <- F_list[[j]]$sum_w_kl_i / ((sum(sqrt(w_kk_i_list[[j]][1:53])))^2)
  
  
  ##############################
  # 3. Calculate Variability of local community i (CV_i_list[[i]])
    CV_i_list[[j]] <- C_list[[j]]$CV_species_i_squared * rho_species_i_list[[j]] # It's not clear whether this should be squared. If not squred, returns a negative value when within-patch species synchrony is negative
  ...
  # 4. Calculate Alpha variability (local community variability weighed for local community biomass; alpha_CV_list[[i]])
    u_m_list[[i]] <- sum(A_list[[j]]$u_i)
    alpha_CV_list[[i]] <- (sum(A_list[[j]]$u_i / u_m_list[[i]] * CV_i_list[[j]] ))^2 # It's not clear whether this should be squared.
  ...
  covMatrix_m_list[[i]] <- cov(another_df_list[[i]]) # create the covariance matrix
  
  w_ii_list[[i]] <- diag(covMatrix_m_list[[i]]) # extract diagonals of the covariance matrix. returns a vector. this is temporal variance of total community CPUE, within each local community.
  
  covMatrix_m_list[[i]][upper.tri(covMatrix_m_list[[i]], diag = TRUE)] <- NA # keep one copy of all the non-diagonals. These are w_ih (temporal covariance of total community biomass between local patches i & h). NB it would be nice to bring in species-level information here, to prevent dominant species from driving the result.
  w_ih_list[[i]] <- as.data.frame(covMatrix_m_list[[i]])
  
  summed_w_ih_list[[i]] <- w_ih_list[[i]] %>% 
    summarise(sum_w_ih = sum(w_ih_list[[i]][,1:ncol(w_ih_list[[i]])], na.rm = T)) # find the sum of all w_ih. calc has been checked

  rho_list[[i]] <- summed_w_ih_list[[i]]$sum_w_ih / ((sum(sqrt (w_ii_list[[i]][1:length(w_ii_list[[i]])] ) ))^2) # Wang & Loreau (2014) claim that standardizing for within-patch covariance accounts for spatial uneveness in biomass. Not sure about this.
  
  Beta1_list[[i]] <- 1 / rho_list[[i]]


  ##############################
  # 6. Calculate Gamma variability
  
  gamma_CV_list[[i]] <- alpha_CV_list[[i]] / Beta1_list[[i]]
...
o <- list(8:10)
q <- list(1:10)
agg_list <- c(a, b, c, d, e, f, g, h, k, l, m, n, o, q)

m <- list(1:4); agg_list <- c(m) # alpha_CV = 6.574154e-05
m <- list(1:3); agg_list <- c(m) # alpha_CV = 2.969732e-05
m <- list(1:10); agg_list <- c(m) # alpha_CV = 1.78654e-05
m <- list(8:10); agg_list <- c(m) # Error: argumemt is not a character vector

WangLoreau_func(shallowCPUEArea2, agg_list) # test the function ... only works for lists beginning with 1
#otherwise  Error: argumemt is not a character vector
