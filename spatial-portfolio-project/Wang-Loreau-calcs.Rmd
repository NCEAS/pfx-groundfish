...
Richness_Area_func <- function(my_df, spatialGrouping){
  ...
  for(i in seq_along(spatialGrouping)) {
    ...
    richness_agg_list[[i]] <- occDat %>%
      ...
      summarize(meanPresenceBySpecies = mean(Mean.avgPresence)) %>% # mean presence of each species, in each year, for each spatialGrouping
      # NOTE mean may still not be the right thing to do ...
      ungroup() %>%
      
      group_by(year.numb) %>%
      summarize(spPresenceByYear = sum(meanPresenceBySpecies)) %>% # spRichness for each year = sum of all spPresence across all species
      ungroup() %>%
...
ggplot(data=richness1, aes(x=Area, y = speciesRichness)) + 
  geom_point(aes(y = speciesRichness), size=2) +
  #geom_line(aes(y = speciesRichness), size = 1) +
