...
deepCPUEArea3 <- deepCPUEArea2 %>%
  mutate(area = fct_recode(area, c("10" = "1", "11" = "2", "12" = "5", "13" = "3", "14" = "4")))


shallowCPUEArea2$depth <- "shallow"
deepCPUEArea3$depth <- "deep"
...
cpue <- shallowCPUEArea2 %>% 
  ...
  dplyr::summarize(annMean = mean(ann.Mean.totalDensity), 
            annCV = annMean/var(ann.Mean.totalDensity)) %>%
  ungroup()
...
BootMean <- shallowDat %>%
  ...
  group_by(area, depth) %>%
  mutate(var_alpha = var(Eff_Num_Sp), var_spRichness = var(Sp_rich), var_beta = var(Exp_B_Div)) %>%
